...
public class AnimalFactory {
  public static func zooFromJSONFileNamed(name:String) -> [Animal]? {
    if let path = NSBundle.mainBundle().pathForResource(name, ofType: "json"), let contentData = NSFileManager.defaultManager().contentsAtPath(path) {
      ...
      if let animals = json["animals"].array {
        for animal in animals {
          if let object = AnimalFactory.animalFromJSON(animal) {
            zoo.append(object)
          }
        }
      }
